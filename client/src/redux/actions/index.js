...
export const getAllCourses = () => async (dispatch) => {
  try {
    const all = await axios.get("http://localhost:3001/courses");
    return dispatch({
...
export const byName = (name) => async (dispatch) => {
  try {
    const byName = await axios.get(`http://localhost:3001/courses?name=${name}`);
    return dispatch({
...
export const getCourseDetail = (id) => async (dispatch) => {
  try {
    const detail = await axios.get(`http://localhost:3001/courses/${id}`);
    return dispatch({
...
export const getTeachers = () => async (dispatch) => {
  try {
    const teachers = await axios.get("http://localhost:3001/teachers");
    return dispatch({
      type: GET_TEACHERS,
...
export const createCourse = (form) => async (dispatch) => {
  try {
    const data = await axios.post("http://localhost:3001/courses", form);
    return dispatch({
      type: CREATE_COURSE,
      payload: data,
    });
...
export const deleteCourse = (id) => async (dispatch) => {
  try {
    await axios.delete("http://localhost:3001/courses");
    return dispatch({
      type: DELETE_COURSE,
    });
  } catch (error) {}
...
export const getReviews = (id) => {
  return async function (dispatch) {
    try {
      var reviews = await axios.get("http://localhost:3001/reviews/" + id);
      return dispatch({
        type: GET_REVIEWS,
...
export const getAllCoursesByTeacher = (userId) => {
  return async function (dispatch) {
    try {
      const response = await axios.get(`http://localhost:3001/courses/teacher/${userId}`);
      return dispatch({
...
export const getCoursesByStudent = (userId) => {
  return async function (dispatch) {
    try {
      const response = await axios.get(`http://localhost:3001/courses/student/${userId}`);
      return dispatch({
